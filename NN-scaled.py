...
'''# Manual Cut
y_man = np.zeros((y_test.shape[0],1))
for i in range(y_test.shape[0]):
    if 123.35 < myy[i] < 127.35:
        y_man[i] = 1

correct = 0
incorrect = 0
for i in range(y_man.shape[0]):
    if y_man[i] == y_test[i]:
        correct += 1
    else:
        incorrect += 1
man_score = correct/(incorrect + correct)
         
print('Manual cut: ' + str(man_score))'''

'''# Get model score on test set
NN_score = model.evaluate(X_test[:,1:], y_test, verbose = 0)[1]

print('Model: ' + str(NN_score))'''